...
func EmitTablesGo(pkgName, outputDir string, tables []*TableDef) error {
	...
	// Individual CREATE TABLE constants.
	var constDefs []Code
	var names []string
	for _, td := range tables {
		constName := "Create" + sqlNameToGoName(td.Name)
	...
	// Creates slice in dependency order.
	var sliceItems []Code
...
func emitInsertFunc(f *File, td *TableDef) {
	...
	f.Comment(fmt.Sprintf("%s converts a %s to %s.", funcName, td.GoType, paramsType))
	f.Func().Id(funcName).Params(funcParams...).Id(paramsType).Block(
		Return(Id(paramsType).Values(dict)),
	)
	f.Line()
}
