...
pkgsql/                        SQL interface for loading packages into SQLite
  ...
  tables.go                    Generated: Table constants + Creates slice
  insert.go                    Generated: Type → sqlc param mapping
...
```sh
cd pkgsql && go run github.com/sqlc-dev/sqlc/cmd/sqlc@v1.29.0 generate
```
...
5. Emit `schema.sql` (CREATE TABLE with inline comments), `query.sql` (INSERT queries for sqlc)
6. Emit `tables.go` (Go constants + `Creates` slice), `insert.go` (type → sqlc param mapping functions)
