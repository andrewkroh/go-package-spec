...
type TransformData struct {
	Transform packagespec.Transform
	Manifest  *packagespec.TransformManifest // nil if absent
...
func readTransform(fsys fs.FS, tPath string, cfg *config) (*TransformData, error) {
	...
	packagespec.AnnotateFileMetadata(transformPath, &td.Transform)

	// Read manifest.yml (optional).
	manifestPath := path.Join(tPath, "manifest.yml")
	manifest, err := readOptionalYAML[packagespec.TransformManifest](fsys, manifestPath, cfg.knownFields)
	if err != nil {
		return nil, fmt.Errorf("reading transform manifest: %w", err)
	}
	if manifest != nil {
		packagespec.AnnotateFileMetadata(manifestPath, manifest)
